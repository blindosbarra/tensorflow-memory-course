...
| La loss di training stessa oscilla selvaggiamente o esplode invece di scendere gradualmente (non un problema di generalizzazione, il modello non impara neanche sul training set) | **Learning rate troppo alto** (Dominio 1, sezione su loss/optimizer) | Passo di aggiornamento dei pesi troppo grande rispetto alla curvatura della loss | Ridurre il learning rate; usare un optimizer con adattamento per-peso (es. Adam) invece di SGD puro; controllare il learning rate prima di rivedere l'architettura del modello |
