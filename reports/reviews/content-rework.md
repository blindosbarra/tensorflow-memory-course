...
inventato il contenuto. La mappatura B1-B3/M1-M5 usa la diagnosi vincolante
...
| M3 | Dataset didattici anticipano la diagnosi | Aggiunti due challenge da 120 righe, generati con seed `20260711`; le lezioni non elencano problemi, quantita' o posizioni. |
...
- `uv sync --extra dev`: PASS; 136 package risolti, package locale ricostruito.
- `ruff check .`: PASS, `All checks passed!`.
- `mypy src`: PASS, nessun problema in 3 file sorgente.
- `pytest`: PASS, 8 test raccolti e passati; warning non bloccante per cache
...
- `mkdocs build --strict`: PASS, build in 0,51 s; informativa non bloccante per
